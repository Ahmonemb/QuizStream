...
import {
  ArrowLeft, Loader2, Plus, RefreshCw, Share2, Tag, Trash2, Video, AlertCircle
} from "lucide-react";
...
import { buildCoursePresentation, formatUploadDate } from "@/lib/coursePresentation";
...
const Courses = () => {
  const {
    user, courses, selectedCourse, selectedCourseId, isLoadingCourses,
    coursesError, selectCourse, refreshCourses, deleteCourse,
  } = useAppState();
  ...
  const correctCheckpointCount = sessionCheckpoints.filter((cp) => cp.status === "completed").length;
  const answeredCheckpointCount = sessionCheckpoints.filter((cp) => cp.status === "completed" || cp.status === "incorrect").length;
  ...
  return (
    <>
              ...
              <>
                ...
                {loadingAI ? (
                  <div className="flex flex-col items-center justify-center py-24 gap-3 bg-card rounded-2xl border border-border">
                    ...
                    <p className="text-muted-foreground">Loading AI interactive session...</p>
                  </div>
                ) : aiError ? (
                    ...
                    <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        ...
                        <p className="mt-0.5 text-sm text-muted-foreground">
                          {coursePresentation.instructor} / {coursePresentation.courseCode} / Uploaded {formatUploadDate(selectedCourse.uploadedAt)}
                        </p>
                  ...
                  <CardHeader className="pb-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <CardDescription>Select an AI lesson to begin an interactive session.</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                    ...
                    {courses.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {courses.map((course) => {
                          ...
                          return (
                            <div key={course.id} className="rounded-xl border border-border p-4 transition-colors hover:bg-muted/40">
                              <div className="flex items-start justify-between gap-3">
                                <button type="button" onClick={() => handleSelectCourse(course.id)} className="min-w-0 flex-1 text-left">
                                ...
                                <Button type="button" variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); void handleDeleteCourse(course.id); }} disabled={isDeleting} className="text-destructive hover:bg-destructive/10">
      ...
      <AddCourseModal
        ...
        onCourseCreated={() => { setActionError(null); setIsLibraryView(false); }}
      />
    </>
  );
};
